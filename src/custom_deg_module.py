...
class Row_Averaging(nn.Module):
    ...
    def sample_unconditional(self, n_sample: int, size, device) -> torch.Tensor:
        """Algorithm 2 in Bansal et al. (2022)"""

        values = torch.FloatTensor(n_sample, 28, 1).uniform_(-0.4, -0.25)
        values[:, 20:28, :] = values[:, 18:28, :].uniform_(-0.5, -0.4)
        values[:, 0:8, :] = values[:, 0:10, :].uniform_(-0.5, -0.4)
        z_t = torch.empty(n_sample, 1, 28, 28)
        for i in range(n_sample):
            for j in range(28):
                z_t[i, 0, j, :] = values[i, j, :]
        ...
        return degraded, z_t_direct, z_t
...
class Col_Averaging(nn.Module):
    ...
    def sample_unconditional(self, n_sample: int, size, device) -> torch.Tensor:
        ...
        values = torch.FloatTensor(n_sample, 1, 28).uniform_(-0.4, -0.15)
        values[:, :, 18:28] = values[:, :, 18:28].uniform_(-0.5, -0.4)
        values[:, :, 0:10] = values[:, :, 0:10].uniform_(-0.5, -0.4)
        z_t = torch.empty(n_sample, 1, 28, 28)
        for i in range(n_sample):
            for j in range(28):
                z_t[i, 0, :, j] = values[i, :, j]
